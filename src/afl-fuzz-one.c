...
u8 fuzz_one_original(afl_state_t *afl) {
  ...
  // default value of minlen = 1
  if (minlen == 1 || afl->mut_crossover == 0) goto skip_crossover;
  ...
  u32   bufread = fread(predbuf, 1, st.st_size, fp);
  fclose(fp);

  u32 nvars = *(uint32_t *)predbuf;
  u32 npreds = *(uint32_t *)(predbuf + 4);
  u32 exp_size = 8 + (4 * nvars) + (nvars * npreds);
  if (bufread != exp_size) {
    printf("ERROR: predmap.bin is corrupted. Skipping crossover\n");
    goto skip_crossover;
  }
      ...
      if (afl->stage_max) {
        for (afl->stage_cur = 0; afl->stage_cur < afl->stage_max;
          ...
          if (mutated_size > 0) {
            ...
            if (!el->afl_custom_fuzz_count) {
              /* If we're finding new stuff, let's run for a bit longer,
                limits permitting. */
