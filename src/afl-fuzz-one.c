...
u8 fuzz_one_original(afl_state_t *afl) {
  ...
  if (afl->mut_crossover == 0) goto skip_crossover;
  if (afl->corrupt_predmap) {
    printf("ERROR: predmap.bin is corrupted. Skipping crossover\n");
    goto skip_crossover;
  }

  printf("running crossover: %u\n", afl->minbuflen);
  ...
  uint16_t *varmap = (uint16_t *)(afl->predbuf + 8);
  uint8_t * predmap = afl->predbuf + 8 + (afl->nvars * 4);
  u64 *     newbuf = (u64 *)DFL_ck_alloc(afl->minbuflen);
  ...
  for (u32 test1 = 0; test1 < afl->queued_items; test1++) {
    ...
    memcpy((u8 *)newbuf, (u8 *)test1buf, afl->minbuflen);

    for (j = 0; j < afl->npreds; ++j) {
      if (afl->queue_buf[test1]->k1_trace[j] == 0 &&
          afl->queue_buf[test2]->k1_trace[j] == 1) {
        for (u32 vid = 0; vid < afl->nvars; ++vid) {
          if (predmap[j * afl->nvars + vid]) {
    ...
    if (common_fuzz_stuff(afl, (u8 *)newbuf, afl->minbuflen)) {
      goto abandon_entry;
    }
    afl->stage_cur += 1;
  }
