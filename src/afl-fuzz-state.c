...
char *power_names[POWER_SCHEDULES_NUM] = {
    "explore", "mmopt", "exploit", "fast", "coe",
    "lin",     "quad",  "rare",    "seek", "custom_one"};
...
void afl_state_init(afl_state_t *afl, uint32_t map_size) {
  ...
  /* Load predmap.bin from the set of constraints to be fuzzed */
  // "predmap.bin" is generated by saadhak, we load it to memory here
  struct stat st;
  int         succ = stat("predmap.bin", &st);
  if (succ == 0) {
    afl->predbuf = ck_alloc(st.st_size);  // (uint8_t *)malloc(st.st_size);
    FILE *fp = fopen("predmap.bin", "rb");
    u32   bufread = fread(afl->predbuf, 1, st.st_size, fp);
    fclose(fp);

    afl->nvars = *(uint32_t *)afl->predbuf;
    afl->npreds = *(uint32_t *)(afl->predbuf + 4);

    u32 exp_size = 8 + (4 * afl->nvars) + (afl->nvars * afl->npreds);
    afl->corrupt_predmap = (bufread != exp_size);

    uint16_t *varmap = (uint16_t *)(afl->predbuf + 8);
    afl->minbuflen =
        varmap[2 * (afl->nvars - 1)] + varmap[2 * (afl->nvars - 1) + 1];
  } else {
    afl->corrupt_predmap = 1;
  }
